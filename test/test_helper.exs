defmodule Fact.TestHelper do
  ...
  def wait_for_event_position_to_be_indexed(instance, position, title, timeout \\ 30_000) do
    receive do
      ...
      {:indexed, pos} ->
        Logger.info("#{title}: #{pos} of #{position}")
        wait_for_event_position_to_be_indexed(position, title, timeout)
    ...
    after
      timeout ->
        ...
        Logger.info("#{title} state = #{inspect(Enum.map(state.indexers, fn {i, b} -> {i, b.position} end))}")      
