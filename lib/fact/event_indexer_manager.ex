defmodule Fact.EventIndexerManager do
  ...
  @impl true
  def init(state) do
    {:ok, supervisor} = DynamicSupervisor.start_link(strategy: :one_for_one, name: EventIndexerSupervisor)
  ...
  @impl true
  def handle_cast({:start_indexer, spec}, %__MODULE__{supervisor: supervisor, indexers: indexers} = state) do
    indexer_key = get_indexer_key(spec)
