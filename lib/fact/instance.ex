defmodule Fact.Instance do
  ...
  def indexer_path(%__MODULE__{} = instance, {indexer_mod, index} = indexer) when is_tuple(indexer) do
  ...
  def indexer_config(%__MODULE__{} = instance, {indexer_mod, _index} = indexer) when is_tuple(indexer) do
  ...
  defp encode_key(value, {:hash, algo}), do: :crypto.hash(algo, to_string(value)) |> Base.encode16(case: :lower)
