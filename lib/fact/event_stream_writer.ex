defmodule Fact.EventStreamWriter do
  ...
  defp ensure_started(event_stream) do
    case Registry.lookup(Fact.EventStreamRegistry, event_stream) do
      [] -> 
        DynamicSupervisor.start_child(Fact.EventStreamWriterSupervisor, {__MODULE__, event_stream})
      _ ->
        :ok
    end
  end
  ...
  def handle_call(
    ...
    if not consistent?(expect, last_pos) do
      {:reply, {:error, {:concurrency, expected: expect, actual: last_pos}}, %{state | idle_timer: idle_timer}}
    else
      ...
      case Fact.EventLedger.commit(enriched_events) do
        {:ok, _last_store_pos} -> {:reply, {:ok, end_pos}, %{state | idle_timer: idle_timer, last_pos: end_pos}}
        {:error, reason} -> {:reply, {:error, reason}, %{state | idle_timer: idle_timer}}
      end
    end
  end
